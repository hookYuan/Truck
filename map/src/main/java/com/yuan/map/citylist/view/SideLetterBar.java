package com.yuan.map.citylist.view;
...
public class SideLetterBar extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < b.length; i++) {
            ...
            paint.setColor(getResources().getColor(R.color.grey500));
            paint.setAntiAlias(true);
            if (i == choose) {
                paint.setColor(getResources().getColor(R.color.grey800));
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (oldChoose != c && listener != null) {
                    if (c >= 0 && c < b.length) {
                        listener.onLetterChanged(c,b[c]);
                        choose = c;
                        invalidate();
                        if (overlay != null){
