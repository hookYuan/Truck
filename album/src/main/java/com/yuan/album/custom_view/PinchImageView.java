...
/**
 * Created by caoyingfu on 16/9/3.
 */
public class PinchImageView extends ImageView {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        getImageBound(bound);
        //相对于屏幕坐标轴的位置
//        Log.i("yuanye","l--"+bound.left+"--r---"+bound.right+"--t---"+bound.top+"--b--"+bound.bottom);
        //控件的位置
//        Log.i("111111","x---"+getLeft()+"---y------"+getRight());
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (bound.width() > getWidth() || bound.height() > getHeight()) {
                    //滑动到边缘（不对事件进行拦截）容差0.5处理
                    if((int)Math.rint(bound.left)== getLeft()||(int)Math.rint(bound.right) == getRight()){
